...
- `/start`：Bot会对自己做基本的介绍。
- `/isbad`：检查用户是否在Bot的黑名单内。
  - 可选参数：`-uids` Bot将回复这些ID是否在黑名单内，可添加多个ID(最多支持50个ID)，用空格隔开，示例 `/is_black -uids 1234567890 88899829221` **(此命令为隐藏命令，并未在BotFather注册，需要手动输入)**
...
- `/channel`：
  - `-refresh`：可选参数，该命令会更新群组的管理员列表到Bot的数据库，用来对调用管理员指令的用户身份做判断。（你通常不需要手动执行此命令，因为Bot被加入群组时会自动执行此命令）。
  - `-rmmsg [true/false]`：可选参数，打开**自动删除进群退群消息**，如聊天列表里的 *某某离开了群组*，*某某加入了群组*，需要提供一个参数来表示开启/关闭，仅支持 [true/false]，当不添加参数时，默认为true
